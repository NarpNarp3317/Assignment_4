...
#include "Enum_printout_direction.h"
...
{
public:
	ListPrinter(int maxLength, HANDLE output_handle, int _highlight_color);
...
public:
	...
	// for now, stock manager and recipe manager convert their list and send it to printer.

	void PrintOutMapList(const map<string, int>& list, Enum_PrintoutDirection direction);// just print out map....// this need to be a template ... fuck
	void PrintOutVectorList(const vector<string> list, Enum_PrintoutDirection direction);

	void PrintOutSearchResults(const vector<SearchResult>& results, Enum_PrintoutDirection direction);// print out the list with color highlight
