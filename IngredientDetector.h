...
public:
	...
	bool CompareIngredientLists(const map<string, int>& A,const map<string, int>& B);// compare list A and B to check if they are same;

	//vector<string> DetectNewIngredients(const map<string, int> &writen_ingredients, const map<string, int>& ingredient_storage);// to detect new ingredient and put them in 
	// --> this is for stock manager
