...
bool IngredientDetector::MakeIngredientList(string writen_ingredients)
{
	...
	*/
	...
	else isvalid = false;// if building name wasnt succesful
...
bool IngredientDetector::CompareIngredientLists(const map<string, int>& A, const map<string, int>& B)
{
	/*
...
/*
vector<string> IngredientDetector::DetectNewIngredients(const map<string, int>& writen_ingredients, const map<string, int>& ingredient_storage)// i guess, this is job for the stock_manager
{
	vector<string> detected_ingredients;
	if (writen_ingredients.size() == 0) return detected_ingredients;
	if (ingredient_storage.size() == 0)
	{
		cout << "Error, no storage ingredient detected!" << endl;
		return detected_ingredients;
	}

	for (const pair <string,int>& ingredient : writen_ingredients)
	{
		 map <string, int>::const_iterator it = ingredient_storage.find(ingredient.first);// there is const_iterator. fuck . 
																							//basically, const_iterator is read only iterator. cannot modify values throw it
		 if (it != ingredient_storage.end())
		 {
...
string IngredientDetector::NameTrimmer(string name)// trim out front and back ' '
{
	...
	if (chunks.empty())// check if there is no chunk of name found
	{
		return"";
	}

	//cout << "the size of chunks vector is = " << chunks.size() << endl; //---- for debug only
